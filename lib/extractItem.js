...
const CATEGORIES = [
  ...
  },
  {
    ...
    keywords: [
      ...
      "abrigo", "bufanda", "guantes", "pantalon corto", "pantalón corto",
      "pantalones cortos", "vaqueros",
      "shirt", "tshirt", "t-shirt", "jacket", "hoodie", "pants", "sweater",
      "cap", "hat", "coat", "scarf", "gloves", "vest", "shorts", "short",
      "jeans",
    ],
  },
  {
...
const NEAR_PREPS = [
  ...
  "afuera de", "fuera de", "outside", "afuera",
];
...
// Cut a string right before the first location preposition it contains (word
// boundaries, checked longest-phrase-first so "in front of" isn't chopped at
// "in") — used to keep the Item text from ever swallowing a preposition or
// the place name after it, whatever preposition it happens to be.
function cutAtLocationPreposition(text) {
  let cutIndex = -1;
  for (const { phrase } of LOCATION_PREPS) {
...
// Time expressions describe WHEN, not WHERE — a captured place-name phrase
// must stop before them ("cerca de la cafetería esta mañana" -> "Near cafetería").
const TIME_WORDS = new Set(["hoy", "ayer", "today", "yesterday", "tonight"]);
const TIME_PHRASES = [
  ["esta", "mañana"], ["esta", "manana"], ["esta", "tarde"], ["esta", "noche"],
  ["this", "morning"], ["this", "afternoon"],
];

function startsTimeExpression(word, nextWord) {
  const w = (word || "").toLowerCase();
  if (TIME_WORDS.has(w)) return true;
  const n = (nextWord || "").toLowerCase().replace(/[.,;:!?()"'¡¿]/g, "");
  return TIME_PHRASES.some(([a, b]) => a === w && b === n);
}

// Grab the noun phrase after a preposition: a few words, stopping at
// punctuation, a conjunction ("y" / "and" / "o" / "or"), or a time expression.
function takePlacePhrase(rest) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (startsTimeExpression(w, words[i + 1])) break;
...
function detectPrepositionLocation(text) {
  for (const { phrase, type } of LOCATION_PREPS) {
    // The place name is optional in the match itself — a preposition with
    // nothing after it ("...left it outside") is still a location for a
    // "near"-type relation (handled below); it's just never one for
    // "inside", where we need an actual place to report.
    const re = new RegExp(
...
// the extracted item phrase, e.g. "Camisa azul en" -> "Camisa azul", or
// "Blue glasses infront" -> "Blue glasses". ("infront of" is covered too, since
// trimming runs right-to-left and "of" is already in this set.)
const TRAILING_CONNECTORS = new Set([
  "en", "in", "cerca", "near", "at", "on", "by", "de", "del",
  "para", "con", "junto", "a", "of", "the", "el", "la", "infront",
]);
...
function detectItem(text, category, color) {
  ...
  for (const trigger of triggers) {
    ...
    // Cut before any location preposition (LOCATION_PREPS), then grab up to
    // the next few words — the item text should never include the
    // preposition or the place name that follows it.
    const beforeLocation = cutAtLocationPreposition(after);
    const words = beforeLocation.split(/\s+/).slice(0, 5);
    const collected = [];
